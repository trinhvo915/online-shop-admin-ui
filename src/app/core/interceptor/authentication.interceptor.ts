...
export class AuthenticationInterceptor implements HttpInterceptor {
  ...
  private handleError(request: HttpRequest<any>, error: HttpErrorResponse): Observable<HttpEvent<any>> {
    // Get message error
    if (error?.error?.errors) {
      const listError = this.getListError(error);
      error.error.message =
        error.error.message != null ? error.error.message + this.getMessageError(listError) : this.getMessageError(listError);
    ...
    if (request.responseType === 'blob' && error.error instanceof Blob) {
      return from(
        Promise.resolve(error).then(async x => {
          ...
          const messageError = this.getMessageError(rsError.errors.map((error: IErrorResponseData) => error.errorMessage));
          if (messageError) {
    ...
    if (error.status === 0 || (error.status >= 400 && error.status < 520 && error.message)) {
  ...
  private getListError(error: HttpErrorResponse): string[] {
    ...
    return responseError.errors.map((error: { errorMessage: any }) => error.errorMessage);
  }
